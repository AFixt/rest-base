...
describe("Setup Standards Integration", () => {
  ...
  describe("Setup Standards Workflow", () => {
    ...
    it("should setup standards in an existing project", (done) => {
      ...
      const child = spawn("node", [setupScript, projectName], {
        stdio: "pipe",
      ...
      child.on("close", (code) => {
        try {
          ...
          expect(fs.existsSync(path.join(tempDir, "docs", "standards"))).toBe(
            true,
          );

          // Check that standards files were copied
          expect(
            fs.existsSync(path.join(tempDir, "docs", "standards", "CLAUDE.md")),
          ).toBe(true);
          expect(
            fs.existsSync(
              path.join(tempDir, "docs", "standards", "global-rules.md"),
            ),
          ).toBe(true);
  ...
  describe("Error Handling and Rollback", () => {
    it("should handle permission errors gracefully", (done) => {
      ...
      const setupScript = path.join(scriptsDir, "setup-standards.js");
      const readOnlyDirName = path.basename(readOnlyDir);

      const child = spawn("node", [setupScript, readOnlyDirName], {
        stdio: "pipe",
        cwd: path.dirname(readOnlyDir),
        env: { ...process.env, NODE_ENV: "test" },
      });
