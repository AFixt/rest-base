    ...
    it('should validate project name and reject invalid names', async () => {
      ...
      for (const invalidName of invalidNames) {
        ...
        try {
          ...
          expect(output.includes('invalid characters') || output.includes('path separators')).toBe(
            true
          );
        }
      }
  ...
  describe('CLI Command Integration', () => {
    it('should create project and then setup standards', async () => {
      ...
      try {
        ...
        expect(finalPackageJson.scripts).toBeDefined();
        expect(finalPackageJson.main).toBe('src/app.js');
      } catch (error) {
        const stdout = error.stdout || '';
        const stderr = error.stderr || '';
        const message = error.message || 'Unknown error';
  ...
  describe('CLI Environment and Configuration', () => {
    ...
    it('should handle custom configuration files', async () => {
      ...
      try {
        ...
        expect(fs.existsSync(path.join(tempDir, 'docs', 'standards', 'global-rules.md'))).toBe(
          true
        );
        expect(fs.existsSync(path.join(tempDir, '.eslintrc.js'))).toBe(true);
      } catch (error) {
        throw new Error(
          `Custom configuration test failed: ${error.message}\nSTDOUT: ${error.stdout}\nSTDERR: ${error.stderr}`
        );
      }
    }, 30000);
  });
  ...
  describe('CLI Error Scenarios', () => {
    it('should handle filesystem permission errors gracefully', async () => {
      ...
      try {
        ...
        expect(error.code === 'EACCES' || (typeof error.code === 'number' && error.code > 0)).toBe(
          true
        );
      } finally {
  ...
  describe('CLI Output and User Experience', () => {
    it('should provide clear progress indicators', async () => {
      ...
      try {
        ...
        const message = error.message || 'Unknown error';
        throw new Error(
          `Progress indicator test failed: ${message}\nSTDOUT: ${stdout}\nSTDERR: ${stderr}`
        );
      }
    }, 35000);
    ...
    it('should provide actionable error messages', async () => {
      ...
      try {
        ...
        const output = (error.stdout || '') + (error.stderr || '');
        expect(output).toContain('ERROR');
        expect(output.includes('path separators') || output.includes('invalid characters')).toBe(
          true
        );
        expect(output.includes('Project name') || output.includes('cannot contain')).toBe(true);
