...
describe('Secure Defaults', () => {
  describe('getSecureDefaults', () => {
    ...
    it('should return api defaults', () => {
      ...
      // Check security defaults
      expect(config.response.defaultHeaders).toHaveProperty('X-Content-Type-Options');
  ...
  describe('validateConfiguration', () => {
    ...
    it('should detect missing express security configuration', () => {
      ...
      expect(result.issues).toContain('Missing Helmet security middleware configuration');
    ...
    it('should detect missing database SSL configuration', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.issues).toContain('SSL not configured for database connection');
    });
  ...
  describe('Default exports', () => {
    ...
    it('should have secure file defaults', () => {
      expect(fileDefaults.upload.maxFileSize).toBeLessThanOrEqual(50 * 1024 * 1024);
      expect(fileDefaults.paths.forbiddenDirectories).toContain('./node_modules/');
      expect(fileDefaults.paths.forbiddenDirectories).toContain('./.git/');
    });

    it('should have secure API defaults', () => {
      expect(apiDefaults.response.defaultHeaders['X-Frame-Options']).toBe('DENY');
      expect(apiDefaults.response.defaultHeaders['X-Content-Type-Options']).toBe('nosniff');
