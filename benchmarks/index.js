...
async function compareResults(baselinePath, currentPath) {
  ...
  for (const bench of comparison.benchmarks) {
    ...
    const execSymbol = execStatus === 'improved' ? '✅' : execStatus === 'degraded' ? '❌' : '➖';
    ...
    const memSymbol = memStatus === 'improved' ? '✅' : memStatus === 'degraded' ? '❌' : '➖';

    console.log(`  Memory Usage: ${memSymbol} ${memChange > 0 ? '+' : ''}${memChange.toFixed(2)}%`);
    console.log(`    Baseline: ${(bench.memory.baseline / 1024 / 1024).toFixed(2)}MB`);
    console.log(`    Current: ${(bench.memory.current / 1024 / 1024).toFixed(2)}MB`);
    ...
    const throughputSymbol =
      throughputStatus === 'improved' ? '✅' : throughputStatus === 'degraded' ? '❌' : '➖';

    console.log(
      `  Throughput: ${throughputSymbol} ${throughputChange > 0 ? '+' : ''}${throughputChange.toFixed(2)}%`
    );
    console.log(`    Baseline: ${bench.throughput.baseline.toFixed(2)} ops/sec`);
...
program.command('list').description('List available benchmark results').action(listResults);
